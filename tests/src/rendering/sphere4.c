...
main()
{
	...
	pexec_struct( PLANE );
	/* Set reflectance properties for the sphere. */
	reflectance.ambient_coef = 1;
	reflectance.diffuse_coef = 1;
	reflectance.specular_coef = 1.0;
	reflectance.specular_exp = 200;
	reflectance.specular_colr.type = PMODEL_RGB;
	reflectance.specular_colr.val.general.x = 1;
	reflectance.specular_colr.val.general.y = 1;
	reflectance.specular_colr.val.general.z = 1;
#ifdef SunPHIGS2.0
	reflectance.transpar_coef = 0;
#endif
	pset_refl_props( &reflectance );
	pexec_struct( SPHERE );
    pclose_struct();
...
#define NUM_LONG_FACETS		40
#define NUM_LAT_FACETS		20
